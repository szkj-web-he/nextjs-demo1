...
export const main = () => {
    ...
    switch (data.direction) {
      case "horizontal":
        ...
        if (data.placement.startsWith("l")) {
          ...
          if (
            isBtnVisible() &&
            x + data.menuSize.width > data.kiteBoundary.right
          ) {
      ...
      case "vertical":
        ...
        if (data.placement.endsWith("b")) {
          ...
          if (
            isBtnVisible() &&
            y + data.menuSize.height > data.kiteBoundary.bottom
          ) {
  ...
  const getHTriangle = (y: number): [number, number] => {
    ...
    if (
      y >= btnTop + data.btnRect.height ||
      y + data.menuSize.height <= btnTop
    ) {
      ty = data.menuSize.height / 2 - data.triangleSize[1] / 2;
    } else {
      const top = Math.max(y, btnTop);
      const bottom = Math.min(
        y + data.menuSize.height,
        btnTop + data.btnRect.height
      );
  ...
  const getVTriangle = (x: number): [number, number] => {
    ...
    if (
      x >= btnLeft + data.btnRect.width ||
      x + data.menuSize.width <= btnLeft
    ) {
      tx = data.menuSize.width / 2 - data.triangleSize[0] / 2;
    } else {
      const left = Math.max(x, btnLeft);
      const right = Math.min(
        x + data.menuSize.width,
        btnLeft + data.btnRect.width
      );
